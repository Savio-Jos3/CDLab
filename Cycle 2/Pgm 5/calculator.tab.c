...
{
    int num;           // For the value of a NUMBER token from the lexer
    struct AST *node;  // For the AST nodes built by the grammar rules
}
/* Line 193 of yacc.c.  */
#line 130 "calculator.tab.c"
	YYSTYPE;
...
#line 60 "calculator.y"
    { (yyval.node) = makeNode("NEG", (yyvsp[(2) - (2)].node), NULL); ;}
    break;
...
#line 62 "calculator.y"
    { (yyval.node) = makeNodeNum((yyvsp[(1) - (1)].num)); ;}
    break;


/* Line 1267 of yacc.c.  */
#line 1376 "calculator.tab.c"
      default: break;
    }
...
/* Helper function to create a new operator node in the tree */
AST* makeNode(char *type, AST *l, AST *r) {
...
/* Helper function to create a new number (leaf) node in the tree */
AST* makeNodeNum(int val) {
    AST *node = (AST*) malloc(sizeof(AST));
    if (!node) {
        yyerror("out of memory");
...
/* Recursively prints the AST with indentation to show the structure */
void printAST(AST *root, int level) {
    if (!root) return;
...
/* The required error handling function */
int yyerror(char *s) {
    printf("Error: %s\n", s);
    return 0;
}
